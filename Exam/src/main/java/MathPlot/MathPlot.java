...
import MathPlot.AreaCalculation.AreaStrategy;
import MathPlot.AreaCalculation.Rectangular;
...
public class MathPlot {
    ...
    public MathPlot() {
        ...
        this.derivativeString = new Constant(0);
    }

    public void setExpression(String expr, ExpressionFormat format) {
        try {
            if (format == ExpressionFormat.AOS) {
                this.expressionString = FormatTranslator.fromAOS(expr);
            } else {
                this.expressionString = FormatTranslator.fromRPN(expr);
            }

            this.expressionString = this.expressionString.simplify();
            this.derivativeString = this.expressionString.calculateDerivative().simplify();
        } catch (Exception e) {
    ...
    public void plot(Canvas canvas, PlotType type) {
        ...
        PlotStrategy strategy;
        if (type == PlotType.Polar) {
            strategy = new Polar();
        } else {
            strategy = new Cartesian();
        }

        if (this.expressionString != null) {
            strategy.plot(pf, this.expressionString, this.derivativeString);
        }
    ...
    public double area(AreaType areaType) {
        ...
        if (areaType == AreaType.Trapezoidal) {
            strategy = new Trapezoidal();
        } else {
    ...
    public List<String> print(ExpressionFormat format) {
        ...
        if (this.expressionString != null) {
            try {
                res.add("Function: " + FormatTranslator.toString(this.expressionString, format));
                res.add("Derivative: " + FormatTranslator.toString(this.derivativeString, format));
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        }
